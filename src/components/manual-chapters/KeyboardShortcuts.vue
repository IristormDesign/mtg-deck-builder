<template>
	<section id="m-keyboard-shortcuts">
		...
		<p>If you’re using MTG Deck Builder on a computer with a physical keyboard (as opposed to on a mobile device), then you have the option to use keyboard shortcuts, by which you press designated keys to perform tasks in the app. You might find keyboard shortcuts convenient because they let you interact with elements on the page faster than by a mouse, trackpad, or touchscreen alone.</p>
		<p>Special keyboard shortcuts are available for <a href="#m-kbsc-deck-page-modes">switching deck page modes</a> and for doing various actions each within the <a href="#m-kbsc-deck-editor">Deck Editor</a>, <a href="#m-kbsc-deck-analyzer">Deck Analyzer</a>, and <a href="#m-kbsc-draw-sim">Draw Simulator</a>.</p>
		<p>Be aware that the app’s keyboard shortcuts cannot work while your web browser is actively focused on any kind of text field, such as the one in the <a href="#m-card-adder">card adder</a>, any <a href="#m-card-quantities">card quantity</a>, or your browser’s web address bar. Under that condition, pressing a letter or digit key will enter that key’s character into the text field instead of doing the shortcut’s action. If you’d want to use a keyboard shortcut but your browser is focused on a text field, first stop the focus by pressing the <kbd>Esc</kbd> key or by clicking on a spot elsewhere on the page.</p>
...
export default {
	components: { KbScDeckPageModes, KbScDeckEditor, KbScDeckAnalyzer, KbScDrawSim }
}
</script>
