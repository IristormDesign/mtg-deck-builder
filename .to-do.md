...
- Whenever a card is added to or removed from a deck or its quantity is changed, the "sort by" select box should go blank (because it's probably no longer accurate to the option it represents). This also allows the user to re-select the same sorting option without needing to switch to another option first.
